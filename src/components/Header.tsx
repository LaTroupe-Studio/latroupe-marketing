  ...
  return (
    <header className={styles.header} style={{ backgroundColor: bgColor, transition: "background-color 0.4s ease" }}>
      <div className={`grid-container ${styles.gridRow}`}>
        ...
        <div className={styles.navProyectos}><button className={styles.navLink} onClick={() => handleClick("proyectos")} style={{color:textColor}}>{content.nav.links[0]?.label}</button></div>
        <div className={styles.navNosotros}><button className={styles.navLink} onClick={() => handleClick("por-que-nosotros")} style={{color:textColor}}>{content.nav.links[1]?.label}</button></div>
        <div className={styles.navContacto}><button className={styles.navLink} onClick={() => handleClick(content.nav.contact.id)} style={{color:textColor}}>{content.nav.contact.label}</button></div>
